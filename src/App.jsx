    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#fff", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ borderBottom: "1px solid #0f0f0f", padding: "11px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#080808", zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ color: "#00ff87", fontSize: 10, letterSpacing: 2, textTransform: "uppercase" }}>NH Analyser</span>
          {planTier && (
            <span style={{ fontSize: 9, color: "#333", background: "#111", padding: "2px 6px", borderRadius: 3, textTransform: "uppercase", letterSpacing: 1 }}>
              {planTier}
            </span>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ color: "#2a2a2a", fontSize: 10 }}>{today}</span>
          {lastUpdated && (
            <span style={{ color: "#2a2a2a", fontSize: 9 }}>
              ↻ {lastUpdated.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}
            </span>
          )}
          {loading && <span style={{ color: "#2a2a2a", fontSize: 9 }}>…</span>}
          {creds && (
            <button
              onClick={() => { setCreds(null); setRaces([]); setPlanTier(null); setLastUpdated(null); setError(""); }}
              style={{ background: "none", border: "1px solid #1a1a1a", color: "#333", fontSize: 9, padding: "3px 8px", borderRadius: 4, cursor: "pointer" }}
            >
              Disconnect
            </button>
          )}
      ...
      {!creds ? (
        <LoginForm onLogin={(u, p) => setCreds({ user: u, pass: p })} error={error} />
      ) : (
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "16px 12px" }}>
          {error && (
            <div style={{ background: "rgba(255,77,77,0.07)", border: "1px solid rgba(255,77,77,0.15)", borderRadius: 6, padding: "12px 16px", marginBottom: 16, color: "#ff6b6b", fontSize: 12 }}>
              {error}
            </div>
          )}

          {loading && races.length === 0 && (
            <div style={{ color: "#2a2a2a", textAlign: "center", padding: 60, fontSize: 12, fontFamily: "'DM Mono', monospace" }}>
              Fetching today's racecards…
            </div>
          )}

          {topSelections.length > 0 && (
            <div style={{ background: "#0c0c0c", border: "1px solid #161616", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
              <div style={{ color: "#00ff87", fontSize: 9, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>
                Top Selections Today
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {topSelections.map((r, i) => (
                  <div key={i} style={{ background: "#111", borderRadius: 5, padding: "6px 10px", borderLeft: `2px solid ${TIER_STYLE[r.tier]?.bg ?? "#333"}` }}>
                    <div style={{ fontSize: 11, color: "#fff", fontFamily: "'DM Mono', monospace" }}>{r.horse || r.horse_name}</div>
                    <div style={{ fontSize: 9, color: "#444", marginTop: 2 }}>{r.off_time} · {r.course} · Score {r.score}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {races.map((race) => (
            <RaceCard key={race.race_id ?? race.off_dt ?? `${race.course}-${race.off_time}`} race={race} />
          ))}
